...
struct dp_handle *dp_fillhandle(pcap_t *phandle) {
  ...
  case (DLT_EN10MB):
    qDebug()<<"Ethernet link detected\n";
    break;
  case (DLT_PPP):
    qDebug()<<"PPP link detected\n";
    break;
  case (DLT_LINUX_SLL):
    qDebug()<<"Linux Cooked Socket link detected\n";
    break;
  default:
    qDebug()<<QString("No PPP or Ethernet link: %1\n").arg(retval->linktype);
...
void dp_parse_ip(struct dp_handle *handle, const dp_header *header,
  ...
  if (DP_DEBUG) {
    qDebug()<<QString("Looking at packet with length %1\n").arg(header->len);
  }
...
void dp_pcap_callback(u_char *u_handle, const struct pcap_pkthdr *header,
  ...
  case (DLT_NULL):
    ...
    break;
  default:
    qDebug()<<QString("Unknown linktype %1").arg(handle->linktype);
    break;
  }
  free(userdata_copy);
}
